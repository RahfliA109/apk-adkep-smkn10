    ...
    public function update(Request $request)
{
    ...
    if ($request->hasFile('gambar')) {
        ...
        if ($user->gambar && Storage::exists('public/storagegambar/' . $user->gambar)) {
            Storage::delete('public/storagegambar/' . $user->gambar);
        }
    
        // Menyimpan file gambar baru ke direktori 'public/storage/storagegambar'
        $filename = time() . '.' . $request->file('gambar')->getClientOriginalExtension();
        $request->file('gambar')->move(public_path('storagegambar'), $filename);
        $user->gambar = 'storagegambar/' . $filename;
